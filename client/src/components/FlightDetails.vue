...
export default {
  ...
  computed: {
    ...
    fromAirport () {
      const orig_str = String(this.flight[5])
      return this.airportCodeToChinese(orig_str)
    },
    toAirport () {
      const orig_str = this.flight[6]
      return this.airportCodeToChinese(orig_str)
    },
    foundTime () {
  ...
  methods: {
    ...
    airportCodeToChinese (value) {
      if (value.includes('ICN')) {
        return '首尔仁川'
      } else if (value.includes('PVG')) {
        return '上海浦东'
      } else if (value.includes('SFO')) {
        return '旧金山'
      } else if (value.includes('JFK')) {
        return '纽约肯尼迪'
